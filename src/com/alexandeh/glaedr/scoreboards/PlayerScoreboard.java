...
public class PlayerScoreboard {
    ...
    public int getScore(Entry entry) {
       /* if (countup) {
            int start = 1 + getBottomWrappers().size();
            int goal = 15 - getTopWrappers().size();

            if (entry instanceof Wrapper) {
                Wrapper wrapper = (Wrapper) entry;
                if (wrapper.getType() == Wrapper.WrapperType.TOP) {
                    goal = getEntries().size() + getBottomWrappers().size() + getTopWrappers().size() + 1;
                    start = getEntries().size() + getBottomWrappers().size() + 1;
                } else {
                    goal = start;
